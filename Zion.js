...
: m.key.participant || m.key.remoteJid;
        ...
        const Ziondev = (m && m.sender && [botNumber, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)) || false;
...
//theme sticker reply
        ...
        if (m.message) {
            ...
            console.log(chalk.bgHex("#000000").bold(`▢ New Message Script By Zion-Dev`));
            console.log(
...
global.menfess = global.menfess ? global.menfess : {}
    ...
    try {
        ...
        const logic = "kamu adalah Xenovia ai, asisten ai yang di buat oleh Zion, kamu bisa mempromosikan link group https://chat.whatsapp.com/FKLOiDRsK4T9DcSvFGxNBx tetapi agak jarang saja jangan terlalu sering mempromosikan nya, kamu adalah asisten ai cerdas yang cantik dan ramah kamu suka emot 🐱😄😆, jangan sering menyapa orang, dan kamu bisa bahasa Jepang tapi jangan kebanyakan jepang";
...
console.error(error);
    ...
    case 'qcstick': {
      ...
      const avatar = await Zion.profilePictureUrl(m.sender, "image").catch(() => 'https://files.catbox.moe/nwvkbt.png')
      const json = {
        type: 'quote',
        format: 'png',
        backgroundColor,
...
case 'pakustad':
case 'pak-ustad': {
  if (!text) return m.reply(`Contoh: ${command} kenapa Zion ganteng`)
